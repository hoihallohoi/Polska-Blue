...
<html>
    ...
    <body>
        ...
        if (isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])) {
            ...
            $emailquery = mysqli_prepare($link, "SELECT emailadress FROM gebruiker WHERE emailadress = ?");
            ...
            $hashquery = mysqli_prepare($link, "SELECT hash FROM gebruiker WHERE hash = ?");
            ...
            $search = mysqli_prepare($link, "SELECT count(emailadress) FROM gebruiker WHERE emailadress = ? AND hash = ? AND activation = ?");
           ...
           // $match = mysqli_stmt_num_rows($search); //aantal treffers tellen
            ...
            if ($match > 0) { //als aantal treffers meer is dan 0 dan wordt de activation op 1 gezet en is het account geactiveerd
                $setActive = mysqli_prepare($link, "UPDATE gebruiker SET activation = ? WHERE emailadress = ? AND hash = ? AND activation = ?");
